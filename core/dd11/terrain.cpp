...
ew::MeshData dd11::generateTerrain(int width, int depth, float scale, int subdivisions)
{
    ...
    for (int z = 0; z < subdivisions; ++z) {
        for (int x = 0; x < subdivisions; ++x) {
            ...
            ew::Vec3 normal(0, 0, 0);
            if (x < subdivisions && z < subdivisions) {
                ew::Vec3 v1 = terrain.vertices[topLeft].pos - terrain.vertices[bottomLeft].pos;
                ew::Vec3 v2 = terrain.vertices[topRight].pos - terrain.vertices[bottomLeft].pos;

                normal = ew::Normalize(ew::Cross(v2, v1));
            }

            terrain.vertices[z * (subdivisions + 1) + x].normal = normal;
...
/*for (int i = 0; i < terrain.vertices.size() - subdivisions; i++)
{
    float bleh = (terrain.vertices[i + 1].pos.y - terrain.vertices[i].pos.y) / width;
    float bleh2 = (terrain.vertices[i + subdivisions].pos.y - terrain.vertices[i].pos.y) / depth;
    terrain.vertices[i].normal = ew::Normalize(ew::Vec3(cos(bleh), sin(bleh) + sin(bleh2), bleh2));
}*/